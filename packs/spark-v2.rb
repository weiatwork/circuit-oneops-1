...
resource 'spark-cluster',
         :except   => ['single'],
         :cookbook => 'oneops.1.spark-cluster-v2',
         :design   => false,
         ...
         :payloads => {
           ...
           'clusterSparkMasters' => {
             ...
             'definition' => '{
               ...
               "direction": "to",
               "targetClassName": "manifest.oneops.1.Spark-cluster-v2",
               "relations": [
                 { "returnObject": false,
                   "returnRelation": false,
                   "relationName": "manifest.DependsOn",
                   "direction": "from",
                   "targetClassName": "manifest.oneops.1.Spark-v2",
                   "relations": [
                     { "returnObject": false,
                       "returnRelation": false,
                       "relationName": "manifest.DependsOn",
             ...
             'definition' => '{
               ...
               "relations": [
                 { "returnObject": false,
                   ...
                   "relationName": "base.RealizedAs",
                   "direction": "from",
                   "targetClassName": "bom.oneops.1.Spark-cluster-v2",
                   "relations": [
                     { "returnObject": true,
                       "returnRelation": false,
...
[
  ...
  {:from => 'artifact-master', :to => 'os-master'},
  {:from => 'library-master', :to => 'os-master'},
  {:from => 'telegraf-master', :to => 'compute-master'},
  {:from => 'telegraf-master', :to => 'os-master'},
...
# managed_via
[ 'spark', 'spark-cassandra', 'user-master', 'artifact-master', 'library-master', 'file-master', 'java-master', 'os-master', 'telegraf-master', 'volume-work-master', 'yarn-master' ].each do |from|
...
[
  ...
  {:from => 'artifact-client', :to => 'os-client'},
  {:from => 'library-client', :to => 'os-client'},
  {:from => 'ansible', :to => 'fqdn'},
  {:from => 'telegraf-client', :to => 'compute-client'},
  {:from => 'telegraf-client', :to => 'os-client'}
].each do |link|
  relation "#{link[:from]}::depends_on::#{link[:to]}",
      :except => [ '_default' ],
      :relation_name => 'DependsOn',
...
# managed_via
[ 'spark-client', 'user-client', 'artifact-client', 'library-client', 'file-client', 'java-client', 'os-client', 'telegraf-client', 'volume-work-client', 'client-yarn', 'ansible' ].each do |from|
