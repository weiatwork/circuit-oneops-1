...
# Updates settings in a Hadoop configuration XML document
#
# INPUT:
# xmlDoc: The document instance to update
# configName: The name of the configuration setting to update
# configValue: The new value for the configuration setting
#
def update_config_value(xmlDoc, configName, configValue)
  xmlNodes = xmlDoc.xpath("/configuration/property[name[text()='#{configName}']]/value")
  if xmlNodes.nil? || xmlNodes.length == 0
    configXMLNode = xmlDoc.xpath("/configuration").first

    # Add a new node
    propNode = Nokogiri::XML::Node.new "property", xmlDoc
    nameNode = Nokogiri::XML::Node.new "name", xmlDoc
    valNode = Nokogiri::XML::Node.new "value", xmlDoc

    nameNode.content = configName
    valNode.content = configValue

    propNode.add_child("\n")
    propNode.add_child(nameNode)
    propNode << "\n"
    propNode.add_child(valNode)
    propNode << "\n"

    configXMLNode << "\n"
    configXMLNode.add_child(propNode)
    configXMLNode << "\n"
  else
    # Update the node
    xmlNodes.each do |xmlNode|
      xmlNode.content = configValue
    end
  end
end