...
# File paths
truststore_file_path = "#{spark_dir}/conf/keystore/pub_truststore_pass"
hive_site_xml_path = "#{spark_dir}/conf/hive-site.xml"
...
if is_client_only && configNode.has_key?('enable_thriftserver') && (configNode['enable_thriftserver'] == 'true')
  ...
  # Read the trust store password from the file
  # if it already exists
  if (File.exist? truststore_file_path)
    public_pass = File.read(truststore_file_path)
  end

  # Read the private password from the hive-site.xml
  # if it already contains one
  if File.exist? hive_site_xml_path
    existingHiveSite = Nokogiri::XML(File.open(hive_site_xml_path))
    passNode = existingHiveSite.xpath("/configuration/property[name[text()='hive.server2.keystore.password']]/value").first
    passNodeText = passNode.content.strip
  ...
  ruby_block "rewrite_hive_site_xml" do
    block do
      ...
      update_config_value(hiveSiteXML, "hive.server2.use.SSL", "true")

      # hive.server2.keystore.path should use a keystore that is unique to Spark
      update_config_value(hiveSiteXML, "hive.server2.keystore.path", "#{spark_dir}/conf/keystore/#{node.workorder.rfcCi.ciName}.keystore")

      # hive.server2.keystore.password should be a password that is unique to Spark
      update_config_value(hiveSiteXML, "hive.server2.keystore.password", "#{private_pass}")

      # Set the parametrized AD auth options
      update_config_value(hiveSiteXML, "hive.server2.thrift.port", "#{configNode['thrift_server_port']}")

      update_config_value(hiveSiteXML, "hive.server2.authentication.ldap.url", "ldap://#{configNode['thrift_ldap_server']}")

      update_config_value(hiveSiteXML, "hive.server2.authentication.ldap.Domain", "#{configNode['thrift_ldap_domain']}")

      # After the values are changed, write out the file.
      File.open(hive_site_xml_path, "w") do |newFile|
