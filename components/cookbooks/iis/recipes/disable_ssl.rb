ssl_keys = [ccs: "current_control_set".camelize, control: "control".camelize, \
   sp: "security_providers".camelize, protocols: "protocols".camelize, tls1: "tls 1.0".upcase, \
...
disable_keys = [
  "%<reg_prefix>s\\%<ccs>s\\%<control>s\\%<sp>s\\%<sl>s\\%<protocols>s\\%<tls1>s\\%<client>s" % ssl_keys,
  "%<reg_prefix>s\\%<ccs>s\\%<control>s\\%<sp>s\\%<sl>s\\%<protocols>s\\%<tls1>s\\%<server>s" % ssl_keys,
...
disable_keys.each do | ssl_registry_key |
